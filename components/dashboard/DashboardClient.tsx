...
export function DashboardClient({ merchantId }: { merchantId: string }) {
  ...
  useEffect(() => {
    ...
    const ordersChannel = subscribeToOrders(merchantId, (payload) => {
      if (payload.eventType === 'INSERT') {
        setOrders((prev) => [payload.new as Order, ...prev]);
      }
    });

    const minlpChannel = subscribeToMinlpRuns(merchantId, (payload) => {
      if (payload.eventType === 'INSERT') {
  ...
  const loadOrders = async () => {
    const data = await getOrders(merchantId);
    setOrders(data);
  };

  const loadLatestMinlpRun = async () => {
    const data = await getLatestMinlpRun(merchantId);
