...
export default function CatalogPage() {
  ...
  const onUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      fd.append('file', f)
      const res = await fetch('/api/merchant/catalog', { method: 'POST', body: fd })
      const j = await res.json()
  ...
  const removeSku = async (sku: string) => {
    ...
    setSaving(true)
    try {
      const res = await fetch(`/api/merchant/catalog?sku=${encodeURIComponent(sku)}`, { method: 'DELETE' })
      const j = await res.json()
      if (res.ok) setItems(j.items || [])
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <a href="/api/merchant/catalog" className="px-4 py-2 rounded-md border text-sm text-gray-700 hover:bg-gray-50">Download JSON</a>
          ...
          <a href="/merchant/forecasts" className="px-4 py-2 rounded-md border text-sm text-gray-700 hover:bg-gray-50">Open Forecasts</a>
        </div>
      </div>
